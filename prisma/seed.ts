...
const createJapanese = async (
  ...
  const japoneseCategory = await prismaClient.category.create({
    data: {
      name: 'Japonesa',
      image_url:
  ...
  for (const item of japoneseRestaurants) {
    const restaurant = await prismaClient.restaurant.create({
      data: item,
    })
    ...
    for (const product of japoneseProducts) {
      await prismaClient.product.create({
        data: product,
      })
